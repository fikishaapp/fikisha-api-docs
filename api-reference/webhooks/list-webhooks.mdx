...
Possible Errors
HTTP CodeError TypeDescription401Authentication RequiredMissing or invalid X-Vendor-ID or X-API-Key.403ForbiddenInsufficient permissions to list webhooks.500Internal Server ErrorServer-side error while retrieving webhooks.
See the Error Handling Guide for details.
Example (Fetch Webhooks)
<CodeGroup>
  <div>
    ```bash
...
// Example: Get all webhooks including inactive ones
listWebhooks(true);
````
...
list_webhooks(include_inactive=True)

```
  </div>
</CodeGroup>
<Info>
  The delivery statistics provide a quick overview of each webhook's reliability. If you notice a webhook with a low success rate, you may want to check for issues with your endpoint or update the webhook URL.
</Info>
```
